...
std::string readFileToString(const std::string& filename) { //chatGPT generated function
...
int main(int argc, char* argv[]){
    ...
    while (getline(inputFile,line)){
        ...
        index = line.find(')');
        year = line.substr(0,index);
        actors= line.substr(index + 1);
        //cout << actors << endl;

        Movie filmElement(name,year,actors);
