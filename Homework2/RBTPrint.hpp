#ifndef _RBTPRINT_HPP_
#define _RBTPRINT_HPP_
...
private:
   ...
   static std::string NodeToString(RBTNode<Key, Type>* node) {
      using namespace std;
      stringbuf buffer;
      ostream os (&buffer);
      if (node->IsRed()) {
         os << "[{" << node->key << ", " << node->value << "} red]";
      } else {
         os << "[{" << node->key << ", " << node->value << "} black]";
      }
      return buffer.str();
   }

   static std::vector<std::string> TreeToLines(RBTNode<Key, Type>* subtreeRoot) {
      using namespace std;
...
public:
   static std::string TreeToString(RBTNode<Key, Type>* subtreeRoot) {
      if (subtreeRoot == nullptr) {
         return "(empty tree)";
      }
